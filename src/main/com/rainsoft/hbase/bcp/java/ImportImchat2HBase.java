 ...
 */
public class ImportImchat2HBase {
    public static void main(String[] args) throws ParseException, IOException {
        //HBase表名
        String tablename = args[0];
        //Hbase表列簇
        String cf = "CONTENT_CHAT";
        //bcp表字段
        String[] columns = FieldConstant.HBASE_FIELD_MAP.get("im_chat");

        String bcpPath = args[1];

        //处理聊天内容的换行问题
        FileUtils.convertFilContext(bcpPath);
        //导入HBase
        importImchatJob(tablename, cf, columns, bcpPath);
     ...
     */
    public static void importImchatJob(String tablename, String cf, String[] columns, String bcpPath) throws ParseException {
        ...
        //按字段切分
        JavaRDD<Row> imchatRowRDD = originalRDD.map(
                (Function<String, Row>) str -> RowFactory.create(str.replace("\\|$\\|", "").split("\\|#\\|"))
        );

        //过滤
        JavaRDD<Row> filterImchatRDD = imchatRowRDD.filter(
                (Function<Row, Boolean>) v1 -> {
                    if (v1.length() < columns.length)
                        return false;
                    else {
                        String capTime = v1.getString(20);
         ...
         *导入HBase
         */
        ImportBcp2HBase.import2HBase(tablename, cf, columns, filterImchatRDD);
    }
}
