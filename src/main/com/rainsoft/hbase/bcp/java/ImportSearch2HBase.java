 ...
 */
public class ImportSearch2HBase {
    public static void main(String[] args) throws ParseException, IOException {
        //HBase表名
        String tablename = args[0];
        //Hbase表列簇
        String cf = "CONTENT_SEARCH";
        //bcp表字段
        String[] columns = FieldConstant.HBASE_FIELD_MAP.get("search");
    ...
    public static void importBcpJob(String tablename, String cf, String[] columns, String bcpPath) throws ParseException {
        ...
        SparkConf conf = new SparkConf()
                .setAppName("import search pcb data into HBase");
        ...
        //按字段切分
        JavaRDD<Row> searchRowRDD = originalRDD.map(
                (Function<String, Row>) str -> RowFactory.create(str.replace("\\|$\\|", "").split("\\|#\\|"))
        );

        //过滤
        JavaRDD<Row> filterSearchRDD = searchRowRDD.filter(
                (Function<Row, Boolean>) row -> {

                    if (row.length() < columns.length)
                        return false;
                    else {
                        String capTime = row.getString(17);
         ...
         *导入HBase
         */
        ImportBcp2HBase.import2HBase(tablename, cf, columns, filterSearchRDD);
    }
}
